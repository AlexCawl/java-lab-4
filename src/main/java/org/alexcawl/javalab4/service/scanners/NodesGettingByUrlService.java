...
@Slf4j
@Service
public class NodesGettingByUrlService implements ScanningStrategy {
    @Autowired
    ...
    public List<WebResource> analyze(String key) {
        return nodeRepository.findAllByURLStartingWith(key)
                .stream()
                .map(element -> {
                ...
                .filter(element -> element.getUrl().startsWith(key))
                .toList();
    }
}
